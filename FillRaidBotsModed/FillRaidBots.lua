...
local function updateRoleConfidence(playerName, class, role, confidenceIncrease, spell)
    ...
    if detectedPlayers[normalizedPlayerName] then
        return  -- Already detected, stop processing
    end
...
local function CheckRaidAuras()
    ...
    for i = 1, GetNumGroupMembers() do
        ...
        if unitName and unitName ~= playerName then  
            ...
            if detectedPlayers[unitName] then
                ...
                -- Class-based detection for non-warriors
                if unitClass == "mage" or unitClass == "warlock" or unitClass == "hunter" then
                    ...
                    -- Detected, skip rest of loop for this unit
                elseif unitClass == "rogue" then
                    detectedPlayers[unitName] = true
                    updateRoleConfidence(unitName, unitClass, "meleedps", 3, "Class Detection")
                    -- Detected, skip rest of loop for this unit
                else
                    -- Check for Greater Armor buff (tank detection) - only for undetected, non-ranged classes
                    for j = 1, 16 do
                        ...
                        if buffName == "Greater Armor" then  
                            ...
                            break  -- Detected as tank, exit buff loop
                        end
                    end

                    -- Warrior detection logic (if not a tank and not already detected)
                    if not detectedPlayers[unitName] and unitClass == "warrior" and not hasTankBuff then
...
function UninviteMember(name, reason)
	...
	if GetRealGroupSize() > 2 then
		SendChatMessage(".pa remove " .. normalizedName, "GUILD")
	else
...
local function CreateRemoveDeadBotsButton()
    ...
    -- Define UpdateDeadBotsButton FIRST
    function UpdateDeadBotsButton()
        ...
        -- Skip if button visibility is disabled in settings
        if not showButtonEnabled then
            removeDeadBotsButton:Hide()
            return
        end
        
        -- NEW: Also skip if auto-kick (isCheckAndRemoveEnabled) is enabled
        if settings.isCheckAndRemoveEnabled then
            removeDeadBotsButton:Hide()
            QueueDebugMessage("Button hidden: Auto-kick is enabled", "debuginfo")
            return
        end
        ...
        for i = 1, GetNumGroupMembers() do
            ...
            if UnitExists(unit) then
                local name = UnitName(unit)
                local isBot = name and string.find(name, "%*")
                local isDead = UnitIsDead(unit) and not UnitIsGhost(unit)
        ...
        -- Simple rule: Button shows if we have dead bots AND total members > 2
        local totalMembers = humanPlayers + aliveBots + deadBots
        local shouldShow = (deadBots > 0) and (totalMembers > 2)
        ...
        -- Debug info
        QueueDebugMessage(string.format(
            "Button check: AutoKick=%s, Humans=%d, AliveBots=%d, DeadBots=%d, Total=%d, Show=%s",
            settings.isCheckAndRemoveEnabled and "ON" or "OFF",
            humanPlayers, aliveBots, deadBots, totalMembers, shouldShow and "YES" or "NO"
        ), "debuginfo")
    end

    -- Now define removeDeadBotsFunction (can use UpdateDeadBotsButton)
    function removeDeadBotsFunction()
        ...
        local humanPlayers = 0
        local aliveBots = 0
        local deadBotsList = {}  -- Store dead bots to remove
        ...
        for i = 1, totalMemberCount do
            ...
            if UnitExists(unit) then
                ...
                if name == playerName then
                    humanPlayers = humanPlayers + 1
                elseif isBot then  -- Bot
                    if isDead then
                        table.insert(deadBotsList, {name = name, unit = unit})
                    else
                        aliveBots = aliveBots + 1
                    end
                else  -- Other human player
                    humanPlayers = humanPlayers + 1
                end
            end
        end
        ...
        QueueDebugMessage(string.format(
            "Removal check: Humans=%d, AliveBots=%d, DeadBots=%d, Total=%d",
            humanPlayers, aliveBots, #deadBotsList, totalMemberCount
        ), "debuginfo")
        
        local deadBotsRemoved = false
        
        -- Apply removal: Remove dead bots but keep at least 2 total members
        if #deadBotsList > 0 then
            -- Calculate how many dead bots we can safely remove
            local totalBots = aliveBots + #deadBotsList
            local totalMembers = humanPlayers + totalBots
            
            -- We need to keep at least 2 members total after removal
            local membersToKeep = 2
            local maxCanRemove = math.max(0, totalMembers - membersToKeep)
            
            -- We can only remove dead bots, not alive ones
            local canRemove = math.min(#deadBotsList, maxCanRemove)
            
            QueueDebugMessage(string.format(
                "Can remove %d/%d dead bots (must keep %d total members)",
                canRemove, #deadBotsList, membersToKeep
            ), "debugremove")
            
            if canRemove > 0 then
                -- Remove the first 'canRemove' dead bots
                for i = 1, canRemove do
                    local botInfo = deadBotsList[i]
                    UninviteUnit(botInfo.name)
                    deadBotsRemoved = true
                    QueueDebugMessage("REMOVED: " .. botInfo.name, "debugremove")
                end
            else
                QueueDebugMessage("Cannot remove any dead bots - would leave <2 members", "debuginfo")
            end
        end
        
        if deadBotsRemoved then
            QueueDebugMessage("Dead bots removed.", "debuginfo")
            -- Update button visibility after removal
            C_Timer.After(0.5, UpdateDeadBotsButton)
        else
            QueueDebugMessage("No dead bots were removed.", "debuginfo")
            UpdateDeadBotsButton()
        end
    end
...
local debugAutoKick = true  -- Set to true to enable auto-kick debug messages in CheckAndRemoveDeadBots funciton
...
local function CheckAndRemoveDeadBots()
    ...
    if autoRemoveEnabled then
        ...
        local totalMembers = CountRealGroupMembers()
        
        if totalMembers <= 2 then return end
                               
        -- Collect ALL dead bot names first
        local deadBotsToRemove = {}
        ...
        for i = 1, maxGroupSize do
            ...
            if UnitExists(unit) then
                ...
                if name and UnitIsDead(unit) and not UnitIsGhost(unit) then
                    if not UnitIsUnit(unit, "player") and UnitIsConnected(unit) and isBotName(name) then
                        table.insert(deadBotsToRemove, name)
                        if debugAutoKick then
                            --DEFAULT_CHAT_FRAME:AddMessage(string.format("|cffff9900[DEBUG]|r Collected dead bot: %s", name))
                        end
                    end
                end          
            end
        ...
        local membersToKeep = 2
        local maxCanRemove = math.max(0, totalMembers - membersToKeep)
        local willRemove = math.min(#deadBotsToRemove, maxCanRemove)
        
        if willRemove == 0 then return end

        -- Batch removal: 3 bots per batch, 0.15s between batches
        local BATCH_SIZE = 3
        local BATCH_DELAY = 0.15
        local numBatches = math.ceil(willRemove / BATCH_SIZE)

        -- Track completed batches for final timing
        local completedBatches = 0
        local totalBatches = numBatches

        for batch = 1, numBatches do
            C_Timer.After((batch - 1) * BATCH_DELAY, function()
                -- Calculate which bots to remove in this batch
                local startIdx = ((batch - 1) * BATCH_SIZE) + 1
                local endIdx = math.min(batch * BATCH_SIZE, willRemove)
                
                -- Build list of bot names for this batch
                local batchBotNames = {}
                for idx = startIdx, endIdx do
                    if deadBotsToRemove[idx] then
                        table.insert(batchBotNames, deadBotsToRemove[idx])
                    end
                end
                
                -- Single line with batch info and all bot names (only if debug enabled)
                if debugAutoKick then
                    local batchTime = GetTime()
                    local batchTimeStr = date("%H:%M:%S")
                    local batchMs = batchTime - math.floor(batchTime)
                    local batchMsStr = string.format("%03d", batchMs * 1000)
                    local batchTimestamp = string.format("%s:%s", batchTimeStr, batchMsStr)
                    
                    DEFAULT_CHAT_FRAME:AddMessage(string.format("|cffff9900[%s]|r |cff00ff00BATCH %d/%d|r %s", 
                        batchTimestamp, batch, numBatches, table.concat(batchBotNames, ", ")))
                end
                
                local removedInBatch = 0
                for index = startIdx, endIdx do
                    local botName = deadBotsToRemove[index]
                    if botName then
                        -- Check if bot is still in the raid by name
                        local botFound = false
                        for j = 1, GetNumGroupMembers() do
                            local unit = IsInRaid() and "raid"..j or (j==1 and "player" or "party"..(j-1))
                            if UnitName(unit) == botName then
                                botFound = true
                                break
                            end
                        end
                        ...
                        if botFound then
                            -- Normalize the name and send remove command
                            local normalizedName = normalizePlayerName(botName)
                            UninviteMember(normalizedName, "dead")
                            removedInBatch = removedInBatch + 1
                            
                            -- Clean up data
                            local normalizedName2 = normalizePlayerName(botName)
                            if playerData[normalizedName2] then
                                ReplaceDeadBot[normalizedName2] = playerData[normalizedName2]
                                playerData[normalizedName2] = nil
                            end
                        elseif debugAutoKick then
                            DEFAULT_CHAT_FRAME:AddMessage(string.format("|cffff9900[%s]|r |cffff0000MISSING|r %s", batchTimestamp, botName))
                        end
                    end
                end
                ...
                -- If this was the last batch, print final timing summary (only if debug enabled)
                if completedBatches == totalBatches and debugAutoKick then
                    local finalTime = GetTime()
                    local totalElapsed = finalTime - startTime
                    local finalTimeStr = date("%H:%M:%S")
                    local finalMs = finalTime - math.floor(finalTime)
                    local finalMsStr = string.format("%03d", finalMs * 1000)
                    local finalTimestamp = string.format("%s:%s", finalTimeStr, finalMsStr)
                    
                    DEFAULT_CHAT_FRAME:AddMessage(string.format("|cffff9900[%s]|r |cff00ffffREMOVAL COMPLETE|r - %d bots in %.3fs", 
                        finalTimestamp, willRemove, totalElapsed))
                end
            end)
        end
    ...
    if showButtonEnabled and not autoRemoveEnabled then
        ...
        for i = 1, GetNumGroupMembers() do
            ...
            if UnitExists(unit) and UnitIsConnected(unit) then
                local name = UnitName(unit)
                if name then
                    local isDead = UnitHealth(unit) == 0 and not UnitIsGhost(unit)
                    if isBotName(name) and isDead then
                        hasDeadBots = true
                    end
                    if not isDead then
                        totalAliveMembers = totalAliveMembers + 1
                    end
                end
            end
        end

        if hasDeadBots and totalAliveMembers >= 2 then
            removeDeadBotsButton:Show()
        else
            removeDeadBotsButton:Hide()
        end
...
local function ProcessMessages()
    -- Process queues only if they have items
    if next(messageQueue) ~= nil then
        ProcessMessageQueue()
    end
    if next(DebugMessageQueue) ~= nil then
...
-- Keep message queue at 0.1s
local messageDelay = 0.25
local deathCheckDelay = 3.5

local function StartMessageTimer()
...
local function StartDeathCheckTimer()
    C_Timer.After(deathCheckDelay, function()
...
local function FillRaid()
    ...
    local function addBot(class)
        ...
        -- Get just the class name without role for display
        local displayClass = plainClass
        if string.find(plainClass, " ") then
            displayClass = string.match(plainClass, "(%a+)")  -- Get first word (class)
        end

        -- Only wait for the 5th bot when total > 4
        if totaly > 4 and botCounter == 5 then
            -- Single combined debug line for bot #5
            QueueDebugMessage("Bot #5: " .. displayClass .. 
                             " | FirstBot: " .. tostring(firstBotName) .. 
                             " | Exists: " .. tostring(firstBotName and UnitExists(firstBotName) or "nil"), "debugfilling")
            
            local waitStartTime = GetTime()
            local MAX_WAIT_TIME_FOR_FIRST_BOT_KICK = 7
            
            -- If firstBotName is nil, skip waiting
            if not firstBotName then
                QueueMessage(".partybot add " .. plainClass, "SAY", true)
                QueueDebugMessage("Added " .. coloredClass .. " (#5 - no firstBot)", "debugfilling")
                return
            end
            
            local function waitForFirstBotRemoval()
                local currentTime = GetTime()
                local elapsed = currentTime - waitStartTime

                
                -- Success: First bot is gone
                if not UnitExists(firstBotName) then
                    QueueDebugMessage("Bot #5 (" .. displayClass .. 
                                     ") after " .. string.format("%.1f", elapsed) .. "s - First bot removed. Waiting 1.25s security", "debugfilling")
                    
                    C_Timer.After(1.25, function()
                        QueueDebugMessage("Now adding bot #5 (.partybot add " .. plainClass .. ")", "debugfilling")
                        QueueMessage(".partybot add " .. plainClass, "SAY", true)
                        QueueDebugMessage("Added " .. coloredClass .. " (#5 - after first bot)", "debugfilling")
                    end)
                    return
                end
                
                -- Timeout: Add anyway
                if elapsed >= MAX_WAIT_TIME_FOR_FIRST_BOT_KICK then
                    QueueMessage(".partybot add " .. plainClass, "SAY", true)
                    QueueDebugMessage("Added " .. coloredClass .. " (#5 - timeout)", "debugfilling")
                    return
                end
                
                -- Continue waiting (check every 0.1 seconds)
                C_Timer.After(0.1, waitForFirstBotRemoval)
            end
        
            waitForFirstBotRemoval()
        else
            -- Single combined line for normal bots
            QueueDebugMessage("Bot #" .. botCounter .. ": " .. displayClass .. " | Total: " .. totaly, "debugfilling")
            
            QueueMessage(".partybot add " .. plainClass, "SAY", true)
            
            -- Single confirmation line
            local suffix = ""
            if botCounter == 5 then
                suffix = " (#5 - no delay)"
            end
            QueueDebugMessage("Added " .. coloredClass .. suffix, "debugfilling")
        end
    end
...
function CreateFillRaidUI()
    ...
    -- Mio
    -- Auto Remove Checkbox (Main UI)
    local autoRemoveMainCheck = CreateFrame("CheckButton", "AutoRemoveMainCheck", FillRaidFrame, "UICheckButtonTemplate")
    autoRemoveMainCheck:SetPoint("TOPLEFT", openSettingsButton, "BOTTOMLEFT", 0, -5)
    autoRemoveMainCheck:SetSize(20, 20)

    autoRemoveMainCheck.text = autoRemoveMainCheck:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    autoRemoveMainCheck.text:SetPoint("LEFT", autoRemoveMainCheck, "RIGHT", -1, 0)
    autoRemoveMainCheck.text:SetText("Remove Dead")
    autoRemoveMainCheck.text:SetFont("Fonts\\FRIZQT__.TTF", 9)

    -- Initialize checkbox with a default value first
    autoRemoveMainCheck:SetChecked(false)

    -- Make sure it's visible initially
    autoRemoveMainCheck:Show()

    -- Function to update checkbox from saved settings
    local function UpdateMainCheckboxFromSettings()
        if FillRaidBotsSavedSettings and FillRaidBotsSavedSettings.isCheckAndRemoveEnabled ~= nil then
            autoRemoveMainCheck:SetChecked(FillRaidBotsSavedSettings.isCheckAndRemoveEnabled)
        else
    ...
    -- Click handler
    autoRemoveMainCheck:SetScript("OnClick", function(self)
        ...
        -- Also update the settings window checkbox if it exists
        if ToggleCheckAndRemoveCheckButton then
            ToggleCheckAndRemoveCheckButton:SetChecked(isChecked)
        end
        ...
        -- NEW: Update button visibility when auto-kick is toggled
        C_Timer.After(0.5, function()
            UpdateDeadBotsButton()
        end)
    end)

    -- Create a frame to listen for ADDON_LOADED event
    local settingsLoaderFrame = CreateFrame("Frame")
    settingsLoaderFrame:RegisterEvent("ADDON_LOADED")

    settingsLoaderFrame:SetScript("OnEvent", function(self, event, addonName)
        if event == "ADDON_LOADED" and addonName == "FillRaidBots" then
            -- Now saved variables should be loaded
            UpdateMainCheckboxFromSettings()
...
-------------------------------------------------------------------------------------------------------------------------------------
	...
	editButton:SetScript("OnClick", function()
        ...
        -- Exiting Edit mode (back to normal view)
			saveButton:Hide()
			saveAsButton:Hide()
			openExportButton:Hide()
			removeButton:Hide()
			...
			currentPresetLabel:Hide()
                    ...
                    -- Show the Remove Dead checkbox
            if autoRemoveMainCheck then
                autoRemoveMainCheck:Show()
            end
            
			fillRaidButton:Show()
			closeButton:Show()
            raidButtonsContainer:Show()
			editButton:SetText("Edit")
		else
        -- Entering Edit mode
			saveButton:Show()
            ...
            -- Hide the Remove Dead checkbox
            if autoRemoveMainCheck then
                autoRemoveMainCheck:Hide()
            end
            
			fillRaidButton:Hide()
			closeButton:Hide()
            raidButtonsContainer:Hide()
...
--Fix for Ossirian the Unscarred and adding alternates for Mandokir (Vilebranch Speaker) and General Rajaxx (Lieutenant General Andorov)
...
function RefillBots()
    ...
    ToggleSoundEffectsVolume("lower")
    DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Starting phased refill...")
    
    -- Count dead bots currently in raid AND add them to ReplaceDeadBot if not already there
    local deadBots = {}
    local deadBotCount = 0
    ...
    for i = 1, totalMembers do
        ...
        if UnitExists(unit) then
            ...
            if isBot and isDead then
                deadBots[name] = true
                deadBotCount = deadBotCount + 1
                
                -- If not already in ReplaceDeadBot, add it
                local normalizedName = normalizePlayerName(name)
                if normalizedName and not ReplaceDeadBot[normalizedName] then
                    -- Try to get class/role from playerData
                    if playerData[normalizedName] then
                        ReplaceDeadBot[normalizedName] = playerData[normalizedName]
                        
                        DEFAULT_CHAT_FRAME:AddMessage(string.format(
                            "|cFFFFFF00[FillRaid]|r Added dead bot to refill list: %s (%s %s)", 
                            name, playerData[normalizedName].ClassNoColor, playerData[normalizedName].role
                        ))
                        playerData[normalizedName] = nil  -- Clear after copying
                    else
                        -- If we don't know class/role, use UnitClass() as fallback
                        local _, englishClass = UnitClass(unit)
                        if englishClass then
                            englishClass = string.lower(englishClass)
                            
                            -- Try to guess role
                            local guessedRole = "meleedps"  -- Default
                            
                            if englishClass == "warrior" then
                                -- Check for tank buffs
                                local isTank = false
                                for j = 1, 16 do
                                    local buffName = UnitBuff(unit, j)
                                    if buffName and string.find(buffName, "Greater Armor") then
                                        isTank = true
                                        guessedRole = "tank"
                                        break
                                    end
                                end
                            elseif englishClass == "paladin" or englishClass == "priest" or 
                                   englishClass == "druid" or englishClass == "shaman" then
                                guessedRole = "healer"
                            elseif englishClass == "mage" or englishClass == "warlock" or englishClass == "hunter" then
                                guessedRole = "rangedps"
                            elseif englishClass == "rogue" then
                                guessedRole = "meleedps"
                            end
                            
                            -- Create data for refill
                            local classColors = {
                                warrior = "|cFFC79C6E",
                                mage = "|cFF40C7EB",
                                warlock = "|cFF8788EE",
                            ...
                            ReplaceDeadBot[normalizedName] = {
                                classColored = classColor .. englishClass .. resetColor,
                                ClassNoColor = englishClass,
                                role = guessedRole
                            }
                            
                            DEFAULT_CHAT_FRAME:AddMessage(string.format(
                                "|cFFFFFF00[FillRaid]|r Guessed class/role for dead bot: %s -> %s %s", 
                                name, englishClass, guessedRole
                            ))
                        else
                            DEFAULT_CHAT_FRAME:AddMessage(string.format(
                                "|cFFFF0000[FillRaid]|r WARNING: Can't detect class for dead bot: %s", 
                                name
                            ))
                        end
                    end
                ...
                DEFAULT_CHAT_FRAME:AddMessage(string.format(
                    "|cFFFFFF00[FillRaid]|r Found dead bot: %s", name
                ))
            end
        end
    end

    -- Now recount ReplaceDeadBot (it may have grown)
    -- Count bots to refill
    local refillCount = 0
    local refillList = {}
    for playerName, data in pairs(ReplaceDeadBot) do
        refillCount = refillCount + 1
        refillList[refillCount] = {name = playerName, data = data}
    end

    if refillCount == 0 then
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Nothing to refill.")
        ToggleSoundEffectsVolume("restore")
        return
    end

    -- Calculate batches
    local firstBatchCount = math.max(0, refillCount - deadBotCount)
    local secondBatchCount = deadBotCount
    
    DEFAULT_CHAT_FRAME:AddMessage(string.format(
        "|cFFFFFF00[FillRaid]|r Strategy: %d dead bots, %d to refill -> Phase1:%d, Phase2:%d",
        deadBotCount, refillCount, firstBatchCount, secondBatchCount
    ))

    -- Phase 1: Add first batch (if any)
    if firstBatchCount > 0 then
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Phase 1: Adding " .. firstBatchCount .. " bots...")
        for i = 1, firstBatchCount do
            local botInfo = refillList[i]
            if botInfo then
                QueueMessage(".partybot add " .. botInfo.data.ClassNoColor .. " " .. botInfo.data.role, "SAY", true)
                DEFAULT_CHAT_FRAME:AddMessage(string.format(
                    "|cFFFFFF00[FillRaid]|r Adding: %s %s", 
                    botInfo.data.ClassNoColor, botInfo.data.role
                ))
                --DEFAULT_CHAT_FRAME:AddMessage(string.format("|cFFFFFF00[FillRaid]|r Adding: %s %s", botInfo.data.ClassNoColor, botInfo.data.role))
            end
        end
        
        -- Wait for first batch to join
        C_Timer.After(3, function()
            -- Phase 2: Kick dead bots (if any)
            if deadBotCount > 0 then
                -- FIRST, verify new bots actually joined
                local function verifyAndKick(attempts)
                    if attempts <= 0 then
                        DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[FillRaid]|r ERROR: New bots didn't join in time!")
                        return
                    end
                    
                    local currentSize = GetRealGroupSize()
                    DEFAULT_CHAT_FRAME:AddMessage(string.format(
                        "|cFFFFFF00[FillRaid]|r Checking: group size = %d (need > 2)", currentSize
                    ))
                    
                    if currentSize > 2 then
                        -- Safe to kick!
                        DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Phase 2: Kicking " .. deadBotCount .. " dead bots...")
                        for botName, _ in pairs(deadBots) do
                            DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Kicking: " .. botName)
                            UninviteMember(botName, "refill cleanup")
                        end
                        
                        -- Phase 3: Add remaining bots after kicks
                        C_Timer.After(2, function()
                            if secondBatchCount > 0 then
                                DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Phase 3: Adding remaining " .. secondBatchCount .. " bots...")
                                for i = firstBatchCount + 1, refillCount do
                                    local botInfo = refillList[i]
                                    if botInfo and ReplaceDeadBot[botInfo.name] then
                                        QueueMessage(".partybot add " .. botInfo.data.ClassNoColor .. " " .. botInfo.data.role, "SAY", true)
                                        --DEFAULT_CHAT_FRAME:AddMessage(string.format("|cFFFFFF00[FillRaid]|r Adding: %s %s", botInfo.data.ClassNoColor, botInfo.data.role))
                                    end
                                end
                            ...
                            C_Timer.After(2, function()
                                ...
                                UpdateReFillButtonVisibility()
                                ToggleSoundEffectsVolume("restore")
                                DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Refill complete!")
                            end)
                        end)
                    else
                        -- Wait and check again
                        C_Timer.After(1, function()
                            verifyAndKick(attempts - 1)
                        end)
                    end
                end
                
                -- Start verification (try for up to 10 seconds)
                verifyAndKick(10)
            else
                -- No dead bots, just clean up
                ReplaceDeadBot = {}
                UpdateReFillButtonVisibility()
                ToggleSoundEffectsVolume("restore")
                DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Refill complete!")
        ...
        -- All slots occupied by dead bots, need to kick first
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r All slots full with dead bots, kicking first...")
        
        -- Kick all but 1 dead bot (keep raid from disbanding)
        local keepOne = true
        local kicked = 0
        for botName, _ in pairs(deadBots) do
            if not keepOne or kicked >= deadBotCount - 1 then
                -- Keep at least 1 bot
                break
            end
        ...
        -- Wait for kicks, then refill
        C_Timer.After(2, function()
            DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[FillRaid]|r Now refilling " .. refillCount .. " bots...")
...
UpdateReFillButtonVisibility()
    ...
    -- Create container for raid instance quick buttons (8 buttons)
    raidButtonsContainer = CreateFrame("Frame", nil, FillRaidFrame)
    raidButtonsContainer:SetPoint("BOTTOM", FillRaidFrame, "BOTTOM", 12, 65)------------------------------------------------------------------------------
    raidButtonsContainer:SetWidth(300)
    raidButtonsContainer:SetHeight(30)

    -- Raid instance buttons data (8 buttons)
    local raidButtonsData = {
        {letter = "N", fullName = "Naxxramas", frameName = "PresetDungeounNaxxramas", presetName = "naxxramasPresets"},
        {letter = "B", fullName = "BWL", frameName = "PresetDungeounBWL", presetName = "bwlPresets"}, 
        {letter = "M", fullName = "MC", frameName = "PresetDungeounMC", presetName = "mcPresets"},
        {letter = "O", fullName = "Onyxia", frameName = "PresetDungeounOnyxia", presetName = "onyxiaPresets", directPreset = "Onyxia"},
        {letter = "A", fullName = "AQ40", frameName = "PresetDungeounAQ40", presetName = "aq40Presets"},
        {letter = "Q", fullName = "AQ20", frameName = "PresetDungeounAQ20", presetName = "aq20Presets"},
        {letter = "Z", fullName = "ZG", frameName = "PresetDungeounZG", presetName = "ZGPresets"},
        {letter = "X", fullName = "Other", frameName = "PresetDungeounOther", presetName = "otherPresets"}
    }

    -- Create raid instance buttons (8 buttons)
    for i, btnData in ipairs(raidButtonsData) do
        local btn = CreateFrame("Button", "FillRaidRaidBtn_"..i, raidButtonsContainer, "UIPanelButtonTemplate")
        btn:SetSize(30, 30)
        btn:SetPoint("LEFT", raidButtonsContainer, "LEFT", (i-1)*35, 0)
        btn:SetText(btnData.letter)
        ...
        btn:SetScript("OnEnter", function(self) 
            ...
            GameTooltip:SetText(btnData.fullName)
            GameTooltip:Show()
        end)
        btn:SetScript("OnLeave", GameTooltip_Hide)
        
        if btnData.directPreset then
            btn:SetScript("OnClick", function()
                PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON)
                ClearAllInputs()
                ApplyPresetValues(btnData.directPreset)
                UpdateInstanceLabels(btnData.fullName, btnData.presetName, btnData.directPreset)
            end)
        else
            btn:SetScript("OnClick", function()
                PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON)
                InstanceButtonsFrame:Hide()
                ClickBlockerFrame:Show()
                ShowPresetFrame(btnData.frameName, btnData.fullName)
                UpdateInstanceLabels(btnData.fullName, btnData.presetName, btnData.fullName)
            end)
        end
    end
    -- Helper functions
    function ClearAllInputs()
        for _, box in pairs(inputBoxes) do
            box:SetNumber(0)
            if box:GetScript("OnTextChanged") then
    ...
    function ApplyPresetValues(presetName)
        local preset = FindPresetByName(presetName)
        if preset and preset.values then
            for role, val in pairs(preset.values) do
                if inputBoxes[role] then
                    inputBoxes[role]:SetNumber(val)
                    if inputBoxes[role]:GetScript("OnTextChanged") then
                        inputBoxes[role]:GetScript("OnTextChanged")(inputBoxes[role])
                    end
                end
            end
        end
    ...
    function UpdateInstanceLabels(instance, presetKey, presetDisplay)
        if currentInstanceLabel then
            currentInstanceLabel:SetText("Instance: "..instance)
            currentInstanceLabel:SetFont("Fonts\\ARIALN.TTF", 10, "OUTLINE")
            currentInstanceName = presetKey
        end
        if currentPresetLabel then
            currentPresetLabel:SetText("Preset: "..presetDisplay)
            currentPresetName = presetDisplay
        end
    end

    function ShowPresetFrame(frameName, displayName)
        local frame = instanceFrames[frameName]
        if frame then
            frame:Show()
            if frame.headerText then
                frame.headerText:SetText(displayName)
            end
        end
    end

    function FindPresetByName(name)
        if not FillRaidPresets or not faction or not FillRaidPresets[faction] then return nil end
        for _, presets in pairs(FillRaidPresets[faction]) do
            for _, p in ipairs(presets) do
                if p.label == name or (p.fullname and p.fullname == name) then
                    return p
                end
            end
        end
...
-- =============== OPTIMIZED EVENT-DRIVEN REMOVAL ===============
local deathDebugEnabled = false
local deadBotsQueue = {}
local queuedBots = {}  -- Fast duplicate checking

local function DebugBotDeath(name, killer)
    if not deathDebugEnabled then return end
    DEFAULT_CHAT_FRAME:AddMessage(string.format(
        "|cffff0000[DEATH]|r %s died | Killer: %s", name, killer or "Unknown"
    ))
end
...
local function ProcessRemovalQueue()
    ...
    local settings = FillRaidBotsSavedSettings or {}
    if not settings.isCheckAndRemoveEnabled then return end
    
    if GetNumGroupMembers() <= 2 then 
        if deathDebugEnabled then
            DEFAULT_CHAT_FRAME:AddMessage("|cffff0000[SAFETY]|r Not starting - only " .. GetNumGroupMembers() .. " members")
        end
        deadBotsQueue = {}
        queuedBots = {}
        return
    ...
    -- Process queue with safety check EACH ITERATION
    local removedCount = 0
    local maxIterations = #deadBotsQueue
    
    for i = 1, maxIterations do
        -- CRITICAL: Check current size BEFORE each removal
        local currentCount = GetNumGroupMembers()
        if currentCount <= 2 then 
            if deathDebugEnabled then
                DEFAULT_CHAT_FRAME:AddMessage("|cffff0000[SAFETY]|r Stopping - only " .. currentCount .. " members remain")
            end
            break  -- Stop the loop, don't process more
        end
        
        local botName = table.remove(deadBotsQueue, 1)
        if not botName then break end
        
        queuedBots[botName] = nil
        
        UninviteMember(botName, "dead")
        removedCount = removedCount + 1
        
        -- Debug
        if deathDebugEnabled then
            DEFAULT_CHAT_FRAME:AddMessage(string.format(
                "|cffff8800[REMOVE]|r %s (#%d) - %d members remain", 
                botName, removedCount, currentCount - 1
            ))
        end
        
        -- ORIGINAL LIMIT still applies (optional)
        local maxRemovals = (currentCount > 5) and 3 or 1
        if removedCount >= maxRemovals then
            if deathDebugEnabled then
                DEFAULT_CHAT_FRAME:AddMessage("|cffff0000[LIMIT]|r Hit max removals per cycle: " .. maxRemovals)
            end
            break
        end
    end
    ...
    -- Continue if queue not empty (with original 0.1s delay)
    if #deadBotsQueue > 0 then
        C_Timer.After(0.1, ProcessRemovalQueue)
    end
end

-- Keep the slash commands at the very end
SLASH_RL1 = "/rl"
SLASH_RL2 = "/reload"
SLASH_RL3 = "/reloadui"
