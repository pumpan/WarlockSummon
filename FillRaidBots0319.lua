...
--
--
-- pumpan: (20260318) RepositionButtonsFromOffset() Clear all button anchors before rebuilding non-horizontal layout.
-- Fixes anchor loop when switching from horizontal back to vertical, where
-- openFillRaidButton could still be anchored to kickAllButton from the old chain.
--
-- Pumpan(20260318):
-- Fixed issue where horizontal button layout was incorrectly following PCPFrame (especially with ElvUI).
-- Previously, all buttons were parented to PCPFrame in fixed mode, which caused:
-- - Buttons to move automatically when PCPFrame moved
-- - Conflicts with manual positioning (offset-based) in horizontal layout
-- - Incorrect positions after reload UI (mainly with ElvUI)
--
-- Fix:
-- - In horizontal layout, all buttons are now parented to UIParent instead of PCPFrame
-- - This makes positioning fully controlled by our own offset logic (GetLeft/GetTop)
-- - After changing parent, we reapply:
--      ApplyButtonStyle() and UpdateButtonSizes()
--   to ensure consistent button sizes (prevents scale mismatch issues)
--
-- Result:
-- - Buttons no longer shift when PCPFrame moves
-- - Consistent positioning after reload
-- - No size differences between buttons
--
